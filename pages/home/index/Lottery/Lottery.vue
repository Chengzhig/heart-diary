<template>
    <view class="conbox">
        <view class="container">
            <!-- 背景 -->
            <image src="../../../../images/bg.png" class="cont" mode=""></image>
  ...
  export default {
    ...
    onLoad: function() {
	  ...
	  /*wx.cloud.callFunction({
		 ...
		 ,success:res=>{
			 console.info('success get Reward', res);
			 if(res.result.status==0){
				 this.chance=res.result.msg.chance
			 }
		 }
		 ,fail:err=>{
			console.info('fail get Reward', err);
		}
	...
	watch:{
		chance: 'onchanceChange',
	},
    methods: {
		onchanceChange(newD, oldD) {
			if(newD==0){
      ...
      playReward()
      {
        ...
        let index = Math.ceil(Math.random()*(this.list.length-1)), duration = 4000
		console.log(index)
