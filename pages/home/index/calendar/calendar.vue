	...
	<view class='all'>
		...
		<view  class="myDateTable">
			<view v-for="(item,j) in days" :key="j" class='dateCell'>
				...
				<view v-else>
					<!-- 已签到日期 -->
					<view v-if="item.isSign == true && item.isSign2 == false " class='cell' style="background:#ffde21;">
						<text>{{item.date}}</text>
					</view>
					<view v-else-if="item.isSign == false && item.isSign2==true" class='cell' style="background: #ff2041;">
						<text>{{item.date}}</text>
					</view>
					<view v-else-if="item.isSign == true && item.isSign2==true" class='cell' style="background: #00ff00;">
						<text>{{item.date}}</text>
					</view>
	...
	export default {
		...
		methods: {
			...
			onJudgeSign() {
				...
				var signs2=this.SignUp2;
				for (var i = 0; i < signs2.length; i++) {
					var current = new Date(signs2[i].replace(/-/g, "/"));
					var year = current.getFullYear();
					var month = current.getMonth() + 1;
					var day = current.getDate();
					day = parseInt(day);
